...
def pp_calc(aim, speed, b, misses, c100, c50, used_mods=mods(), combo=0xFFFF, score_version=1, c300=0xFFFF):
    ...
    old_combo_break = math.pow(combo, 0.8) / math.pow(b.max_combo, 0.8)

    combo_power = ((1 - acc) ** 0.5) + 0.2
    new_combo_break = (combo ** combo_power) / (b.max_combo ** combo_power)
    ...
    old_pp = math.pow(math.pow(1, 1.1) + math.pow(1, 1.1) + math.pow(acc_value, 1.1),
                      1.0 / 1.1) * final_multiplier
